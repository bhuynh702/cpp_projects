...
void inPuzzle(char userInput, string puzzle, char answers[]) 
    {
    /*
    This function checks if the input is found in the puzzle,
    updates the puzzle, and checks input validation 
    */
    ...
    for (unsigned int i = 0; i < puzzle.length(); i++) // iterates through the puzzle
    {
        if (userInput == answers[i]) // if userInput is found in answers 
        {
            alreadyGuessed = true; // make the bool flag true 
        }else if (userInput == puzzle[i]){ // if userInput is equal to a letter in the puzzle 
            found = true; // make the bool flag true 
            letterCount++; // update the letter count 
            answers[i] = puzzle[i]; // update the underscores to letters
        }
    }
    /*
    if-else if-else below checks input validation
    */
...
int main () 
{
    ...
    */
    ...
    printPuzzle(answers, selectedPuzzle.length()); // prints underscores first

    char input = getInput(); // gets user input 
    
    inPuzzle(input, selectedPuzzle, answers); // checks if input is in the puzzle and valid, then updates underscore to letter
