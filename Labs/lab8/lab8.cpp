...
LinkedList::~LinkedList() { // Destructor for linked list
  while(m_head != nullptr) {      // While the list is not empty, Remove the first node
    RemoveFront();
  }
}
...
void LinkedList::InsertEnd(string data) {
  ...
  //list is not empty
    ...
    lastNode = nullptr; //pointers aren't needed anymore, so set to nullptr
    newNode = nullptr;
  }
}
...
void LinkedList::Display() {
  ...
  //List has Node(s)
    ...
    Node* temp = m_head; // temp goes through the entire list to print m_info for each Node
